...
[Z, P] = AS_TFRtoZ(dmid,labmark,dof,[]);
% P1 = P(Z>0);
% P2 = P(Z<0);
% [h1 pi1] = fdr(P1, 0.05);
% [h2 pi2] = fdr(P2, 0.05);
...
%     figure;imagesc(Doutshowtemp)
    ...
    axis off;
    saveas(H,[outdir,filesep,['z',num2str(i),'.fig']])
    ...
    print(H,[outdir,filesep,['z',num2str(i),'.tif']],'-dtiff','-r300')
    close(H)
end
...
%     figure;imagesc(Doutshowtemp)
    ...
    imagesc(Doutshowtemp1,[-dshownum,dshownum*2]);colormap(colormapshow);
    axis off;
%     saveas(H,[outdir,filesep,['y',num2str(i),'.fig']])
%     set(H,'PaperPositionMode','manual');
%     set(H,'PaperUnits','inch')
%     XSIZE = size(Doutshowtemp1,2);
%     YSIZE = size(Doutshowtemp1,1);
...
%     YSIZEU = YSIZEnew(FACTORS);    
%     set(H,'Paperposition',[1,1,XSIZEU/300,YSIZEU/300]);
%     print(H,[outdir,filesep,['y',num2str(i),'.tif']],'-dtiff','-r300')
%     close(H)
end
...
%     figure;imagesc(Doutshowtemp)
    ...
    imagesc(Doutshowtemp1,[-dshownum,dshownum*2]);colormap(colormapshow);
    axis off;
%     saveas(H,[outdir,filesep,['x',num2str(i),'.fig']])
%     set(H,'PaperPositionMode','manual');
%     set(H,'PaperUnits','inch')
%     XSIZE = size(Doutshowtemp1,2);
%     YSIZE = size(Doutshowtemp1,1);
...
%     YSIZEU = YSIZEnew(FACTORS);    
%     set(H,'Paperposition',[1,1,XSIZEU/300,YSIZEU/300]);
%     print(H,[outdir,filesep,['x',num2str(i),'.tif']],'-dtiff','-r300')
%     close(H)
end
