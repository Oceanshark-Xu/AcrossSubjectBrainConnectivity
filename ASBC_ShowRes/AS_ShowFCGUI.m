...
%%
% Hshow.IO
Hshow.IO_inputpb = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.05 0.675 0.1 0.3],'style','text','string','Overlay image');
Hshow.IO_inputsel = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.85 0.675 0.1 0.3],'style','pushbutton','string','Select');
Hshow.IO_inputed = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.2 0.675 0.6 0.3],'style','edit');

Hshow.IO_tempputpb = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.05 0.35 0.1 0.3],'style','text','string','Template image');
Hshow.IO_tempputsel = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.85 0.35 0.1 0.3],'style','pushbutton','string','Select');
Hshow.IO_tempputed = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.2 0.35 0.6 0.3],'style','edit','string',templatepath);

Hshow.IO_Outputpb = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.05 0.025 0.1 0.3],'style','text','string','Outputdir');
Hshow.IO_Outputsel = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.85 0.025 0.1 0.3],'style','pushbutton','string','Select');
pwdpath = pwd;
Hshow.IO_Outputed = uicontrol('parent',Hshow.IO,'units','norm','pos',[0.2 0.025 0.6 0.3],'style','edit','string',pwdpath);
...
Hshow.Other_type = uicontrol('parent',Hshow.Other,'units','norm','pos',[0.525,0.8,0.2,0.15],'style','text','string','R T F P?');
...
Hshow.Other_typeE = uicontrol('parent',Hshow.Other,'units','norm','pos',[0.525,0.6,0.2,0.2],'style','edit','string','T');
...
% DbgRe(DbgRe<30*0.9) = 0;
% DbgRe(DbgRe>=85)= 85;
% for i = 1:size(DbgRe,1)
%     Sliceval = squeeze(DbgRe(i,:,:));
%     save([Hshow.pat,filesep,'TempForOrigShow',filesep,'Xslice',num2str(i),'.mat'],'Sliceval');
% end
% for i = 1:size(DbgRe,2)
%     Sliceval = squeeze(DbgRe(:,i,:));
%     save([Hshow.pat,filesep,'TempForOrigShow',filesep,'Yslice',num2str(i),'.mat'],'Sliceval');
% end
% for i = 1:size(DbgRe,3)
%     Sliceval = squeeze(DbgRe(:,:,i));
%     save([Hshow.pat,filesep,'TempForOrigShow',filesep,'Zslice',num2str(i),'.mat'],'Sliceval');
% end
% clear Sliceval;
...
% set(Hshow.Other_ShowTYPE1,'callback',{@ASmapshowType1,Hshow});
% set(Hshow.Other_ShowTYPE2,'callback',{@ASmapshowType2,Hshow});
% set(Hshow.Other_ShowTYPE3,'callback',{@ASmapshowType3,Hshow});
%
...
if strcmpi(Types,'P')    
    ...
    [DOUTSHOWaxial,DOUTSHOWcornoral,DOUTSHOWsagittal,DOUTSHOWaxialPOS,...
        DOUTSHOWcornoralPOS,DOUTSHOWsagittalPOS,DOUTSHOWaxialNEG,DOUTSHOWcornoralNEG,...
        DOUTSHOWsagittalNEG,dshownum] = AS_MapFCshow3sliceUP(outdir,inputdir,templatedir,pthrv,colmaxt,CLUSNUM);
...
%     save temp07062
    [DOUTSHOWaxial,DOUTSHOWcornoral,DOUTSHOWsagittal,DOUTSHOWaxialPOS,...
        DOUTSHOWcornoralPOS,DOUTSHOWsagittalPOS,DOUTSHOWaxialNEG,DOUTSHOWcornoralNEG,...
        DOUTSHOWsagittalNEG,dshownum,ptempshow0001]=AS_MapFCshow3sliceU(outdir,inputdir,templatedir,pthrv,Types,dof1,dof20,colmaxt,CLUSNUM);
    set(Hshow.Other_PthE,'string',num2str(ptempshow0001));
    if Showtype1==1
        SagShowmap = DOUTSHOWsagittal(:,:,SliceSag);
        CorShowmap = DOUTSHOWcornoral(:,:,SliceCor);
        AxiShowmap = DOUTSHOWaxial(:,:,SliceAxi);
...
Pval = get(Hshow.Other_PthE,'string');
if strcmpi(Types,'P')
    Pvalnew = str2num(Pval);
    set(Hshow.Other_Threshold1,'string',num2str(1-Pvalnew));
%     set(Hshow.Other_Threshold1,'string',num2str(1-Pval));
else
    if strcmpi(Types,'F')&&length(DOFv)<2
        error('wrong Degree of Freedom for F stastical analysis');
    end
    ...
    pval = str2num(Pval);
%     [Z, P] = AS_TFRtoZ(dmid,Types,dof1,dof20);
    [Z,out] = AS_PtoTRF(pval,Types,dof1,dof20);
    set(Hshow.Other_Threshold1,'string',num2str(out));
end
end
