function AS_MapFCshow_multisliceU(DATASHOW,sliceorder,outdir,OutnameLab,colormapshow,climsuse,printlab)
...
Doutshowtemp2 = zeros(size(DATASHOW,1)*NW,size(DATASHOW,2)*NL);
for i = 1:length(sliceorder)
    Doutshowtemp1 = squeeze(DATASHOW(:,:,sliceorder(i)));
    IX = i;
    POSXT = POSX(IX);
    POSYT = POSY(IX);
