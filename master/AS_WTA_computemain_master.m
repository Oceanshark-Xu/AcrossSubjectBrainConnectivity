...
if any(vseed.dim-vinput.dim) % 当皮层ROI与输入数据不一致的时候
    Maskimg = vseed.fname;
...
if any(vtarget.dim-vinput.dim) % 当皮层下ROI与输入数据不一致的时候 % 此处考虑到数据的不完整性，配准可能存在问题
...
IND1 = find(IND0~=0);
save(Rmatsavename,'r','p','IND1','vtarget','indexstarget'); % 将记录信息保存为computeval.mat，其中IND1为非零点
%%
